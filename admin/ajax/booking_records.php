...
if(isset($_POST['get_bookings']))
{
    ...
    // Pagination logic
    $limit = 2; // 2 records per page for demo (change to 10/20)
    $page = $frm_data['page'];
    $start = ($page-1) * $limit;

    // Fetch History: (Booked & Arrived) OR (Cancelled & Refunded) OR (Payment Failed)
    ...
    while($data = mysqli_fetch_assoc($limit_res)){
        ...
        <tr>
            ...
            <td>
                <button type='button' onclick='download_pdf($data[booking_id])' class='btn btn-outline-success btn-sm fw-bold shadow-none'>
    ...
    if($total_rows > $limit){
        ...
        if($page!=1){
            ...
            $pagination .= "<li class='page-item'><button onclick='change_page(".($page-1).")' class='page-link shadow-none'>Prev</button></li>";
        }

        $disabled = ($page==$total_pages) ? "disabled" : "";
        $pagination .= "<li class='page-item $disabled'><button onclick='change_page(".($page+1).")' class='page-link shadow-none'>Next</button></li>";
