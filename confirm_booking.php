...
<div class="container">
  <div class="row">
    ...
    <div class="col-lg-7 col-md-12 px-4">
        <?php 
            // MODIFIED: Corrected Image Paths
            $room_thumb = SITE_URL.'images/rooms/1.jpg'; // Correct default path
            $thumb_q = mysqli_query($conn, "SELECT * FROM `room_images` WHERE `room_id`='$room_data[id]' AND `thumb`='1'");

            if(mysqli_num_rows($thumb_q) > 0){
                $thumb_res = mysqli_fetch_assoc($thumb_q);
                // MODIFIED: Use SITE_URL + images/ + Folder + Filename
                $room_thumb = SITE_URL.'images/'.ROOMS_FOLDER.$thumb_res['image'];
            }

            echo <<<data
            <div class="card p-3 shadow-sm rounded">
                <img src="$room_thumb" class="img-fluid rounded mb-3" style="height: 200px; width: 100%; object-fit: cover;">
